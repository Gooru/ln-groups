...
import org.gooru.groups.responses.transformers.ResponseTransformerBuilder;
...
class HttpServerResponseWriter implements ResponseWriter {
  ...
  HttpServerResponseWriter(RoutingContext routingContext,
    ...
    this.transformer = ResponseTransformerBuilder.build(message.result());
  }
  ...
  public void writeResponse() {
    final HttpServerResponse response = this.routingContext.response();
    // First set the status code
    this.writeHttpStatus(response);
    // Then set the headers
    this.writeHttpHeaders(response);
    // Then it is turn of the body to be set and ending the response
    this.writeHttpBody(response);
  }

  private void writeHttpBody(HttpServerResponse response) {
    if (this.transformer.transformedStatus() != HttpStatus.NO_CONTENT.getCode()) {
      final String responseBody = ((this.transformer.transformedBody() != null)
          && (!this.transformer.transformedBody().isEmpty()))
              ? this.transformer.transformedBody().toString()
              : null;
  ...
  private void writeHttpHeaders(HttpServerResponse response) {
    Map<String, String> headers = this.transformer.transformedHeaders();
  ...
  private void writeHttpStatus(HttpServerResponse response) {
    response.setStatusCode(this.transformer.transformedStatus());
  }
}
