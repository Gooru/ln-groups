...
import org.gooru.groups.responses.writers.ResponseWriterBuilder;
...
class RouteFailureConfigurator implements RouteConfigurator {
  ...
  private static void handleFailures(RoutingContext frc) {
    ...
    if (currentThrowable instanceof io.vertx.core.json.DecodeException) {
      ...
      frc.response().setStatusCode(HttpStatus.BAD_REQUEST.getCode()).end("Invalid Json payload");
      ...
      frc.response().setStatusCode(HttpStatus.ERROR.getCode()).end("Internal error");
    }
  }
